...
export async function buildSeededFinanceTransactions(tenantId = 'production') {
  ...
  const salesOrders = await getOrders(tenantId);
  const salesRevenue = salesOrders.reduce((sum, order) => sum + Number(order.amount_total || order.totalAmount || 0), 0);
...
export async function buildModuleActivityEvents(tenantId = 'production') {
  ...
  const finance = buildSeededFinanceAgingData(tenantId);
  const sales = await getOrders(tenantId);
  ...
  return [
    {
      ...
      sourceModel: 'inventory.picking',
      sourceId: warehouse.shipments[0]?.shipmentId || 'ship-501',
    ...
    },
    {
      ...
      sourceModel: 'accounting.move',
      sourceId: finance.vendorLines[0]?.invoiceId || 'ap-001',
    ...
    },
    {
      ...
      sourceModel: 'sales.order',
      sourceId: sales[0]?.id || 'ord-001',
    ...
    },
    {
      ...
      sourceId: purchase.purchases[0]?.poId || 'PO-1001',
    ...
    },
    {
      ...
      sourceModel: 'inventory.inventory',
      sourceId: 'cc-batch-01',
