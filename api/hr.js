...
async function handleEmployees(req, res, tenantId, rest) {
  ...
  if (req.method === 'GET' && rest.length === 0) {
    const result = await pool.query(
      `SELECT id, employee_code, first_name, last_name, email, phone, department, job_title, hire_date, salary, active, created_at
  ...
  if (req.method === 'POST' && rest.length === 0) {
    const { employee_code, first_name, last_name, email, phone, department, job_title, hire_date, salary } =
      req.body || {};
    const result = await pool.query(
      ...
      [tenantId, employee_code, first_name, last_name, email, phone, department, job_title, hire_date, salary || 0]
    );
