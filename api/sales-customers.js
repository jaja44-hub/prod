 ...
 * Direct Neon DB handler for customers (Vercel serverless compatible)
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      const query = `
        INSERT INTO customers (tenant_id, customer_code, name, email, phone, city, country, credit_limit, balance, active)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 0, true)
        RETURNING *
      `;
      
      const result = await pool.query(query, [tenantId, customer_code, name, email, phone, city, country, credit_limit || 0]);
