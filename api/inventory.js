...
async function createProduct(req, res, tenantId) {
  const { product_code: skuFromBody, sku: skuFromBodyAlt, name, description = '', category = '', quantity = 0, unit_price = 0 } = req.body || {};
  // Use product_code if provided, otherwise fall back to sku
  const sku = skuFromBody !== undefined ? skuFromBody : skuFromBodyAlt;
