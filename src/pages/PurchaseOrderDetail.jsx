      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded shadow-sm max-w-3xl">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold">
            {id === 'new' ? t('createPurchaseOrder') : `${t('order')} ${order?.name || id}`}
          </h2>
          <button onClick={() => navigate('/purchases')} className="text-sm text-gray-500">
            {t('back')}
          </button>
        </div>
        ...
        {loading ? (
          ...
          <form onSubmit={submit} className="space-y-4">
            <div>
              <label className="block text-xs text-gray-600 mb-1">{t('vendor')}</label>
              <select
            ...
            <div className="flex justify-between items-center">
              <button type="button" onClick={() => navigate('/purchases')} className="px-3 py-1 border rounded">
                {t('cancel')}
              </button>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-3 bg-gray-50 dark:bg-gray-900 rounded">
                <div className="text-xs text-gray-500 mb-1">{t('vendor')}</div>
                  ...
                  <tbody>
                    {(order?.order_lines || []).map((line) => (
                      <tr key={line.id} className="border-b last:border-b-0">
                        ...
                        <td className="py-2">{line.product_qty != null ? line.product_qty : '—'}</td>
