    ...
    async function loadData() {
      ...
      try {
        ...
        if (activeTab === 'accounts') {
          const result = await client.get('/api/finance/accounts?tenant_id=tenant_default', { service: 'finance' }).catch((e) => { console.error('Accounts fetch error:', e); return null; });
          if (mounted && result) {
            console.log('Accounts result:', result);
            setAccounts(result.data || []);
          }
        } else if (activeTab === 'journals') {
          const result = await client.get('/api/finance/journal?tenant_id=tenant_default&entry_type=JOURNAL', { service: 'finance' }).catch(() => null);
          if (mounted && result) setJournals(result.data || []);
        } else if (activeTab === 'payments') {
          const result = await client.get('/api/finance/journal?tenant_id=tenant_default&entry_type=PAYMENT', { service: 'finance' }).catch(() => null);
          if (mounted && result) {
